...
def build_home(root: str = "") -> str:
    ...
    faqs_html = (
        ...
        if faqs
        else "<p>No FAQs available.</p>"
    )

    timeline_sorted = sorted(timeline, key=lambda ev: str(ev.get("event_date", "")), reverse=True)
    timeline_html = (
        "<ul>"
        + "".join(
        ...
        + "</ul>"
        if timeline
        else "<p>No timeline events available.</p>"
    )
    ...
    current_as_of = status.get("current_as_of", "2026-07-09")
    if not isinstance(current_as_of, str):
        current_as_of = "2026-07-09"
    return render_page(title="Home", body=body, current_as_of=current_as_of, root=root)


def source_links(urls: object) -> str:
...
def _faq_sources_html(urls: object) -> str:
...
def build_timeline(root: str = "") -> str:
    ...
    if not events:
        ...
        events_sorted = sorted(events, key=lambda ev: str(ev.get("event_date", "")), reverse=True)
        events_html = "".join(
    ...
    return render_page(
        title="Timeline", body=render("timeline.html", {"events": Safe(events_html)}), root=root
    )
...
def build_waivers(root: str = "") -> str:
    ...
    if not waivers:
        ...
        rows_html = "".join(rows)
        table = f"<table><thead><tr><th>Type</th><th>Party</th><th>Scope</th><th>Start</th><th>End</th><th>Source</th></tr></thead><tbody>{rows_html}</tbody></table>"
    return render_page(
...
def build_approvals(root: str = "") -> str:
    ...
    if not approvals:
        ...
        rows_html = "".join(rows)
        table = f"<table><thead><tr><th>Producer</th><th>Brand/Family</th><th>Device</th><th>Start</th><th>End</th><th>Source</th></tr></thead><tbody>{rows_html}</tbody></table>"
    return render_page(
